...
void lock(int i){
	...
	/*
	Memfences prevent the reordering of load and store instructions by the compiler. A
	  memfence is required here so that the below assignment to ENTERING is not reordered
	  before the previous assignment to ENTERING.
	*/
	mfence();
	...
	NUMBER[i] = 1 + max;
    /*
	A memfence is required here so that the memory instructions for NUMBER and ENTERING below
	  are not reordered to occur before the assignment above.
	  NOTE: No additional mfences are required because all accesses to NUMBER or ENTERING below 
	    are loads only, so the reordering of these are unnecessary. Since these global variables are the 
		only critical ones not protected by a lock, and there no other function loads and stores to 
		these arrays, no other memfences are required.
	*/
	mfence();
